	...
	namespace core
	{
		namespace memory
		{
			template <typename _Lock = pilo::core::threading::dummy_mutex>
				class compactable_dynamic_memory_pool
				: public dynamic_memory_pool<_Lock>
			{
			public:
				typedef _Lock                                   lock_type;
				typedef dynamic_memory_pool<_Lock>				base_type;
			...
			protected:
				bool _need_compact_nolock() const
				{
					...
					return (piece_count > 3) && (free_obj_count / (::pilo::int_r) this->m_steps) >= (piece_count / 2);
				}
			...
			public:
                compactable_dynamic_memory_pool() : dynamic_memory_pool<_Lock>(0, 0)
                {
                }

                compactable_dynamic_memory_pool(size_type unit_size, size_type n_steps) : dynamic_memory_pool<_Lock>(unit_size, n_steps)
                {

                }

                virtual ~compactable_dynamic_memory_pool()
                {
                }

				::pilo::i32_t init(size_type unit_size, size_type n_steps)
				{
					return this->base_type::init(unit_size, n_steps);
				}
