...
//                }
            ...
            namespace detail
            {
                ...
                inline std::string replace_ordinals(
                    std::string text,
                ...
                inline std::pair<cron_int, cron_int> make_range(
                    ...
                    ::pilo::i8_t tz)
                {
                    ...
                    if (PMI_INVALID_TIMESTAMP == ::pilo::core::datetime::datetime_to_timestamp_by_second(date, tz))
                    ...
                    ::pilo::i8_t tz)
                {
                    for (size_t i = 0; i < marked_fields.size(); ++i)
                    {
                        if (marked_fields.test(i))
                            reset_field(date, static_cast<cron_field>(i), tz);
                    }
                }
                    ...
                    ::pilo::i8_t tz)
                {
                    ...
                    {
                        add_to_field(date, next_field, 1, tz);
                        reset_field(date, field, tz);
                        next_value = next_set_bit(target, minimum, maximum, 0);
                    }

                    if (INVALID_INDEX == next_value || next_value != value)
                    {
                        set_field(date, field, static_cast<int>(next_value),tz);
                        reset_all_fields(date, marked_fields,tz);
                    }
                    ...
                    ::pilo::i8_t tz)
                {
                    ...
                    while (
                        ...
                        && count++ < maximum)
                    {
                        add_to_field(date, cron_field::day_of_month, 1, tz);
                    ...
                    ::pilo::i8_t tz)
                {
                    ...
                    {
                        res = find_next<Traits>(cex, date, dot,tz);
                        if (!res) return res;
                    }
            ...
            {                
                ...
                cronexpr* cexp = new cronexpr;

                detail::set_cron_field(fields[0], cexp->seconds, Traits::CRON_MIN_SECONDS, Traits::CRON_MAX_SECONDS);
                detail::set_cron_field(fields[1], cexp->minutes, Traits::CRON_MIN_MINUTES, Traits::CRON_MAX_MINUTES);
                detail::set_cron_field(fields[2], cexp->hours, Traits::CRON_MIN_HOURS, Traits::CRON_MAX_HOURS);

                detail::set_cron_days_of_week<Traits>(fields[5], cexp->days_of_week);

                detail::set_cron_days_of_month<Traits>(fields[3], cexp->days_of_month);

                detail::set_cron_month<Traits>(fields[4], cexp->months);
            ...
            static std::tm cron_next(cronexpr const& cex, std::tm, ::pilo::i8_t tz)
            {
                ...
                if (!detail::find_next<Traits>(cex, date, date.tm_year,tz))
                    return {};

                time_t calculated = ::pilo::core::datetime::datetime_to_timestamp_by_second(date, tz);
                if (PMI_INVALID_TIMESTAMP == calculated) return {};

                if (calculated == original)
                {
                    add_to_field(date, detail::cron_field::second, 1, tz);
                    if (!detail::find_next<Traits>(cex, date, date.tm_year, tz))
                        return {};
                }
            ...
            template <typename Traits = cron_standard_traits>
            static std::time_t cron_next(cronexpr const& cex, std::time_t date, ::pilo::i8_t tz)
            {
                date += (tz * PMI_SECS_OF_HOUR);
                std::tm val;                
                std::tm* dt = ::pilo::core::datetime::xpf_get_gmtime(&val ,&date);
                if (dt == nullptr) return PMI_INVALID_TIMESTAMP;
                
                time_t original = ::pilo::core::datetime::datetime_to_timestamp_by_second(*dt, tz);
                if (PMI_INVALID_TIMESTAMP == original) return PMI_INVALID_TIMESTAMP;

                if (!detail::find_next<Traits>(cex, *dt, dt->tm_year,tz))
                    return PMI_INVALID_TIMESTAMP;

                time_t calculated = ::pilo::core::datetime::datetime_to_timestamp_by_second(*dt, tz);
                if (PMI_INVALID_TIMESTAMP == calculated) return calculated;

                if (calculated == original)
                {
                    add_to_field(*dt, detail::cron_field::second, 1, tz);
                    if (!detail::find_next<Traits>(cex, *dt, dt->tm_year,tz))
                        return PMI_INVALID_TIMESTAMP;
                }

                return ::pilo::core::datetime::datetime_to_timestamp_by_second(*dt, tz);
            }

            template <typename Traits = cron_standard_traits>
            static std::chrono::system_clock::time_point cron_next(cronexpr const& cex, std::chrono::system_clock::time_point const& time_point, ::pilo::i8_t tz) {
                return std::chrono::system_clock::from_time_t(cron_next<Traits>(cex, std::chrono::system_clock::to_time_t(time_point), tz));
            }
        }
