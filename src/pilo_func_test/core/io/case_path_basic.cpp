		...
		namespace core
		{
			namespace io
			{
				...
				static ::pilo::err_t create_complex_dir_hirechy()
				{
					...
					::pilo::core::io::path rt("test_fs\\cdh", ::pilo::core::io::path::unknow_length, 0, ::pilo::predefined_pilo_path::tmp);
					...
					err = p0.set(test_str_0, ::pilo::predefined_pilo_path::tmp);
					if (err != PILO_OK)
						return err;
				...
				{	
					...
					::pilo::core::io::path rt2("test_fs\\cdh2\\", ::pilo::predefined_pilo_path::tmp);
					if (rt2.invalid())
					{
					...
					::pilo::core::io::path rt2_0("test_fs\\cdh2\\测文件符号\\", ::pilo::predefined_pilo_path::tmp);
					if (rt2_0.invalid())
					{
						return ::pilo::mk_perr(PERR_NULL_PARAM);
					}
					...
					::pilo::core::io::path p1_link("test_fs\\cdh\\目录1\\目录2\\目录3\\cdh2.lnk", ::pilo::predefined_pilo_path::tmp);
					...
					::pilo::core::io::path p2_link("test_fs\\cdh\\目录1\\目录2\\一号文件abc.txt.lnk", ::pilo::predefined_pilo_path::tmp);
...
//                }
				...
				int case_path_basic(::pilo::core::testing::func_test_case* p_case)
				{
					::pilo::core::io::path p0;
					if (!p0.invalid())					
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "empty path should be invalid");
					
					p0.set("a");
					if (p0.invalid())					
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 shoud be valid");					
					int vret = 0;
					if ((vret = ::pilo::core::testing::validate_path(&p0, "a", 1, "a", nullptr, nullptr, "a", ::pilo::core::io::path::relative)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed %d", vret);
					

					::pilo::err_t err = p0.set("|");
					if (err == PILO_OK)					
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed |");

					err = p0.set("ab");
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed ab");
					if ((vret = ::pilo::core::testing::validate_path(&p0, "ab", 2, "ab", nullptr, nullptr, "ab", ::pilo::core::io::path::relative)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed ab %d", vret);
					...
					err = p0.set("dir0/dir1/ddd.txt", ::pilo::predefined_pilo_path::home);
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed ddd");					

					err = p0.set("../../../dir0/dir1/ddd.txt", ::pilo::predefined_pilo_path::cwd);
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed ddd");
					...
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "set long (目录1\\..\\..\\目录2\\目录3\\目录4) path");
					if ((vret = ::pilo::core::testing::validate_path(&p0, "目录1\\..\\..\\目录2\\目录3\\目录4", (::pilo::pathlen_t)::pilo::core::string::character_count("目录1\\..\\..\\目录2\\目录3\\目录4"), "目录4", nullptr, "目录1\\..\\..\\目录2\\目录3", "目录4", ::pilo::core::io::path::relative)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed chn dir path %d", vret);

					err = p0.set("\\\\?\\Z:");
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed \\\\?\\Z:");
					if ((vret = ::pilo::core::testing::validate_path(&p0, "\\\\?\\Z:", 6, "\\\\?\\Z:", nullptr, nullptr, "\\\\?\\Z:", ::pilo::core::io::path::absolute)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed \\\\?\\Z: %d", vret);
					...
					err = p0.set("\\\\?\\C");
					if (err == PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed ! \\\\?\\C %d", vret);
					...
					err = p0.set("f:");
					if (err != PILO_OK)					
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed f:");					
					if ((vret = ::pilo::core::testing::validate_path(&p0, "\\\\?\\f:", 6, "\\\\?\\f:", nullptr, nullptr, "\\\\?\\f:", ::pilo::core::io::path::absolute)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed f: %d", vret);					

					err = p0.set("a\\");
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed a\\");
					if ((vret = ::pilo::core::testing::validate_path(&p0, "a", 1, "a", nullptr, nullptr, "a", ::pilo::core::io::path::relative)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed %d", vret);
					...
					err = p0.set("E:d");
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed E:d");
					if ((vret = ::pilo::core::testing::validate_path(&p0, "\\\\?\\E:\\d", 8, "d", nullptr, "\\\\?\\E:", "d", ::pilo::core::io::path::absolute)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed E:d %d", vret);
					...
					err = p0.set("a/b");
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed a/b");
					if ((vret = ::pilo::core::testing::validate_path(&p0, "a\\b", 3, "b", nullptr, "a", "b", ::pilo::core::io::path::relative)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed a/b %d", vret);
					...
					err = p0.set("E:\\a\\b\\c\\..\\..\\d\\e.txt");
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed E:\\a\\b\\c\\..\\..\\d\\e.txt");
					if ((vret = ::pilo::core::testing::validate_path(&p0, "\\\\?\\E:\\a\\d\\e.txt", 16, "e.txt", "txt", "\\\\?\\E:\\a\\d", "e", ::pilo::core::io::path::absolute)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed E:\\a\\b\\c\\..\\..\\d\\e.txt %d", vret);

					err = p0.set("E:\\a\\b\\c\\..\\..\\d\\e\\\\");
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "E:\\a\\b\\c\\..\\..\\d\\e\\\\");
					...
					err = p0.append("f\\g\\h");
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "append f\\g\\h");
					...
					if ((vret = ::pilo::core::testing::validate_path(&p0, "dir0\\dir1\\dir2\\dir3\\dir4", 24, "dir4", nullptr, "dir0\\dir1\\dir2\\dir3", "dir4", ::pilo::core::io::path::relative)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed dir0/dir1/dir2/dir3/dir4 %d", vret);
					...
					if (err != PILO_OK)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed /a/b/c/d/e.txt");
					if ((vret = ::pilo::core::testing::validate_path(&p0, "/a/b/c/d/e.txt", 14, "e.txt", "txt", "/a/b/c/d", "e", ::pilo::core::io::path::absolute)) != 0)
						return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "p0 val failed /a/b/c/d/e.txt %d", vret);
				...
				int case_get_paths(::pilo::core::testing::func_test_case* p_case)
				{
					...
					if (err != PILO_OK)
					{
						return p_case->error(::pilo::mk_perr(PERR_GET_PATH_FAIL), "get cwd failed");
					}
					if (enbuf.invalid())
					{
						return p_case->error(::pilo::mk_perr(PERR_GET_PATH_FAIL), "get cwd failed");
					...
					if (enbuf.is_dynamic())
					{
						return p_case->error(::pilo::mk_perr(PERR_GET_PATH_FAIL), "get cwd should not alloc memory");
					}
					...
					if (smbuf.invalid())
					{
						return p_case->error(::pilo::mk_perr(PERR_GET_PATH_FAIL), "get cwd invalid");
					}
					if (!smbuf.is_dynamic())
					{
						return p_case->error(::pilo::mk_perr(PERR_GET_PATH_FAIL), "get cwd should not alloc memory");
					...
					//{
					//	return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "cwd is not dir type");
					//}
				...
				int case_paths_attr(::pilo::core::testing::func_test_case* p_case)
				{
					...
					//if (ret.invalid())
					//{
					//	return p_case->error(::pilo::mk_perr(PERR_TESTCASE_FAIL), "get node type failed of symlink");
					//}
