...
#               endif
            ...
            pilo::i32_t dynamic_astring::_assign(const char* str)
            {
                ...
                {
                    _m_size = string_util::length(str);
                }
                
                if (_m_capacity <= _m_size)
                {
                    MP_SAFE_FREE(_m_pdata);
                    _m_capacity = M_ALIGN_SIZE(_m_size + 1, sizeof(void*));
                    _m_pdata = (char*)malloc(_m_capacity);
                    if (_m_pdata == nullptr)
                    {
                        return pilo::EC_INSUFFICIENT_MEMORY;
                    }
                ...
                if (_m_size != string_util::copy(_m_pdata, _m_capacity, str, _m_size))
                {
                    this->reset();
                    return pilo::EC_UNDEFINED;
                }
