    ...
    namespace core
    {
        namespace container
        {
            ...
            public:
                ...
                inline void clear()
                {
                    pilo::core::threading::mutex_locker<lock_type> locker(m_mutex);
                ...
                inline bool insert_after(value_type* prev, value_type* node)
                {
                    pilo::core::threading::mutex_locker<lock_type> locker(m_mutex);

                    if (prev == nullptr) //add a head node
                    {
                        if (m_head == nullptr)
                        {
                            node->m_next = nullptr;
                    ...
                    else //normal case
                    {
                        ...
                        if (prev == m_tail) //add after the last node
                        {
                            m_tail = node;
                        }
                    }
            ...
            protected:
                ...
                lock_type               m_mutex;
            private:
                M_DISABLE_COPY(singly_linked_selflist);
            };       
