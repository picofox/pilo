...
int main(int argc, char *argv[])
{
    ...
    ::pilo::core::container::sorted_dynamic_array<PlayerRunningRecord, ref_gel_compare_less<PlayerRunningRecord>> sda;
    sda.clear();

        sda.clear();
        for (int i = 0; i < 200; i++)
        {
            PlayerRunningRecord r;
            int v = rand() % 200;
            r.m_nTimeCost = v;
            r.m_tickTimeStamp = i;
            // printf("add %d\n", v);
        ...
        {
            if (sda[i].m_nTimeCost > sda[i + 1].m_nTimeCost)
            {
                printf("error\n");
                exit(-1);
            }

            printf("%d = %d\n", i, sda.at(i).m_nTimeCost);
        }

        PlayerRunningRecord ri;
        ri.m_nTimeCost = 113;

        size_t posi = sda.insert(ri);
        printf("pos i = %d\n",posi);
...
//         }
//     
//     PlayerRunningRecord rf;
//     rf.m_nTimeCost = 41;
//     size_t pos = sda.find(rf);
//     printf("43 is in %d pos\n",pos);
